...
if ($_POST)
{
    ...
    if (isset($_POST["fullName"]) && isset($_POST["lastName"])) {
        if (!isset($_POST["fullName"]) && !isset($_POST["lastName"]))
        {
            $output = json_encode(array('type'=>'error', 'text' => 'Input fields are empty!'));
            die($output);
            ...
            $user_Name = filter_var($_POST["fullName"], FILTER_SANITIZE_STRING) . " " . filter_var($_POST["lastName"], FILTER_SANITIZE_STRING);
        }
    }
    //education
